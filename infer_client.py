...
def create_pod_and_get_id(name, image_name, gpu_type_id, ports, container_disk_in_gb, env_vars):
    try:
        pod = runpod.create_pod(name=name, image_name=image_name, gpu_type_id=gpu_type_id,
                                ports=ports, container_disk_in_gb=container_disk_in_gb, env=env_vars,network_volume_id='wiv2uv12s6')
...
def convert_voice1(url,file_paths,spk_id):
    """
    Synchronously uploads multiple files.
...
def download_and_save_mp3_1(pod_id, audio_id, save_path):
    """
    Downloads an MP3 file using the given 'audio_id' and saves it locally.
...
def main(file_path,spk_id):
    ...
    url="https://jg29c26vedrcj2-5000.proxy.runpod.net/convert_voice"
    audio_id =convert_voice1(url,file_path,spk_id)
    if audio_id:
        download_and_save_mp3_1(url, audio_id, save_path)
